...
const LineChart: React.FC<LineChartProps> = ({ data, title, unit, color, type, yAxisTitle }) => {
  ...
  const calculateAverage = () => {
    if (type === 'single') {
      return (data.reduce((sum, item) => sum + (item.value ?? 0), 0) / data.length).toFixed(1);
    } else {
      return {
        in: (data.reduce((sum, item) => sum + (item.carbonIn ?? 0), 0) / data.length).toFixed(0),
        out: (data.reduce((sum, item) => sum + (item.carbonOut ?? 0), 0) / data.length).toFixed(0)
      };
    }
  };
          ...
          {type === 'single' ? (
            ...
            <div className="space-y-1">
              <p className="text-gray-900 text-base font-bold">
                In: {typeof average === 'object' && 'in' in average ? average.in : ''}{unit}
              </p>
              <p className="text-gray-900 text-base font-bold">
                Out: {typeof average === 'object' && 'out' in average ? average.out : ''}{unit}
              </p>
            </div>
          )}
        </div>
